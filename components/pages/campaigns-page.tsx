...
import { Megaphone, Pause, Pencil, Play, Plus, RotateCcw, Search, Send, Trash2, Upload } from "lucide-react";
...
type CampaignStatus = "DRAFT" | "AUDIO_UPLOADED" | "PENDING_REVIEW" | "APPROVED" | "REJECTED" | "CHANGES_REQUESTED" | "SCHEDULED" | "RUNNING" | "PAUSED" | "CANCELED" | "COMPLETED" | "FAILED";
type Group = { id: string; name: string; _count: { contacts: number } };
type Campaign = { id: string; name: string; description: string | null; audioUrl: string | null; audioAssetId: string | null; contactGroupId: string; status: CampaignStatus; startTime: string | null; retryEnabled: boolean; retryCount: number; ivrSettings: unknown; contactGroup: Group; createdBy: { name: string }; company?: { name: string } };
type PaginatedCampaigns = { items: Campaign[]; pagination: { page: number; pages: number; total: number; limit: number } };
type CampaignForm = { name: string; description: string; contactGroupId: string; status: CampaignStatus; startTime: string; retryEnabled: boolean; retryCount: string; ivrSettings: string; audioUrl: string; audioAssetId: string };

const STATUSES: CampaignStatus[] = ["DRAFT", "AUDIO_UPLOADED", "PENDING_REVIEW", "APPROVED", "REJECTED", "CHANGES_REQUESTED", "SCHEDULED", "RUNNING", "PAUSED", "CANCELED", "COMPLETED", "FAILED"];
const EMPTY_FORM: CampaignForm = { name: "", description: "", contactGroupId: "", status: "DRAFT", startTime: "", retryEnabled: false, retryCount: "0", ivrSettings: "{}", audioUrl: "", audioAssetId: "" };
...
export function CampaignsPage() {
  ...
  async function saveCampaign(event: FormEvent) {
    ...
    try {
      ...
      const payload = { ...form, companyId: companyId || undefined, audioAssetId: uploadedAudio.audioAssetId, description: form.description || null, startTime: form.startTime ? new Date(form.startTime).toISOString() : null, retryCount: Number(form.retryCount), ivrSettings };
  ...
  async function startCampaign(campaign: Campaign) {
    try {
      const result = await apiRequest<{ queuedJobs: number }>(`/api/campaigns/${campaign.id}/start`, jsonRequest("POST"));
      setNotice(`${t("modules.campaigns.started")}: ${result.queuedJobs}`);
      await loadCampaigns();
    } catch (value) {
  ...
  async function toggleCampaign(campaign: Campaign) {
    ...
    try {
      await apiRequest(`/api/campaigns/${campaign.id}/${action}`, jsonRequest("POST"));
      setNotice(t(`modules.campaigns.${action}d`));
      await loadCampaigns();
    } catch (value) {
      setError(value instanceof Error ? value.message : t("modules.common.error"));
    }
  ...
  async function submitCampaign(campaign: Campaign) {
    try {
      await apiRequest(`/api/campaigns/${campaign.id}/submit`, jsonRequest("POST"));
      setNotice(t("modules.campaigns.submitted"));
      await loadCampaigns();
...
function campaignVariant(status: CampaignStatus): "default" | "secondary" | "success" | "warning" | "destructive" {
  ...
  if (status === "SCHEDULED" || status === "PAUSED" || status === "PENDING_REVIEW" || status === "CHANGES_REQUESTED") return "warning";
  return "secondary";
}
...
function audioDuration(file: File) {
  return new Promise<number>((resolve, reject) => {
    ...
    audio.onloadedmetadata = () => { URL.revokeObjectURL(url); resolve(Math.ceil(audio.duration)); };
    audio.onerror = () => { URL.revokeObjectURL(url); reject(new Error("Audio metadata could not be read")); };
    audio.src = url;
  });
}
